...
days produces NA because it overcorrelates with other variables.Indeed, variety and ays are equivalent, we can't have both in the same model.

-> -> -> En fait, il faudrait modifier variety pour que ça reflete vraiment la marque de la pomme !
...
fit.all.step <- step(lm(`rate of advance` ~ strain+variety+ weight+radius, data=data))
fit.2ways.step <- step(lm(`rate of advance` ~ strain+variety, scope = ~strain + variety + weight+radius, data=data))
```

Stepwise all procedure arrive au meme resultat : only strains and variety.
Auriane : je n'arrive pas a faire step dans les 2 sens, qqn sait pq ??? Et aussi je ne trouve pas la fonction stepAIC
...
plot(foster.hsd)
```
Auriane : je ne sais pas faire marcher cette fonction apparemment.... ???
...
#general plots
plot(residuals(fit.sv))
qq<-qqnorm(residuals(fit.sv))
stem(residuals(fit.sv))
plot(fit.sv)
```
